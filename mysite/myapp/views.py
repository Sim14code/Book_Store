...
# from rest_framework import status
# from rest_framework.permissions import IsAuthenticated
# from .models import User, Book, LikedBook
...
#             liked_books = LikedBook.objects.filter(user=user_instance)

#             books_data = BookSerializer([liked_book.book for liked_book in liked_books], many=True)
...
#             if LikedBook.objects.filter(user=user_instance, book=book).exists():
#                 return Response({"message": "You have already liked this book"}, status=status.HTTP_400_BAD_REQUEST)

#             liked_book = LikedBook(user=user_instance, book=book)
#             liked_book.save()
...
#             liked_book = LikedBook.objects.filter(user=user_instance, book=book).first()
#             if not liked_book:
#                 return Response({"message": "You have not liked this book yet"}, status=status.HTTP_400_BAD_REQUEST)

#             liked_book.delete()
#             return Response({"message": "Book unliked successfully"}, status=status.HTTP_200_OK)
...
#             # Parse JSON body
#             data = json.loads(request.body)
#             amount = data.get("amount")  # Access the 'amount' field
...
#             print(f"Received amount in paise: {amount_in_paise}")
#             response = JsonResponse({"message": "Order created!"})
